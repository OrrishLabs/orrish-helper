    ...
    return (
        <div style={{ margin: "20px 200px" }}>
            <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-around", gap: "20px" }}>
                ...
                <Button disabled={textBoxValue.trim().length === 0} onClick={() => encrypt()} color="primary" variant="contained" style={{ textTransform: 'none' }}>Encrypt</Button>
                <Button disabled={textBoxValue.trim().length === 0} onClick={() => decrypt()} color="primary" variant="contained" style={{ textTransform: 'none' }}>Decrypt</Button>
            </div>
            {shouldResultDisplay &&
                <div style={{ margin: "20px", alignContent: "center" }}><span>The {buttonClicked}ed value is :  </span>
                    <span >{cryptedValue}</span><Tooltip title="Copy this step to clipboard.">
