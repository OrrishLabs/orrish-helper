...
const EditableTable = (props: any) => {
    ...
    const onRowDragEnd = (event: RowDragEndEvent) => {
        let items = event.node.parent.allLeafChildren.map((e) => {
            return { ...e.data, id: e.childIndex + 1 };
        });
    ...
    const onDelete = (event: CellEvent) => {
        let items: Step[] = event.node.parent.allLeafChildren.map((e) => {
            return { ...e.data };
        });
    ...
    const [columnDefs] = useState([
        ...
        { field: 'help' },
        {
            ...
            editable: false,
            cellStyle: { padding: '0' },
            cellRenderer: function (e: CellEvent) {
                return (
                    <IconButton size="small" color="warning" onClick={() => onDelete(e)}>
                        <Delete />
                    </IconButton>
                );
            }
        },
        {
            ...
            cellRenderer: function (e: CellEvent) {
                return (
                    <IconButton size="small" color="success" onClick={() => onAddRow(e)}>
                        <Add />
                    </IconButton>
                );
            }
